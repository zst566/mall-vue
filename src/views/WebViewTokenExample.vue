  ...
  <div class="webview-token-example">
    ...
    <div class="token-section">
      ...
      <div class="action-buttons">
        <button @click="handleGetToken" :disabled="loading" class="btn primary">
          <span v-if="loading">⏳ 获取中...</span>
          <span v-else>🔐 从小程序获取 Token</span>
        </button>

        <button @click="handleLogin" :disabled="loading" class="btn secondary">
          <span v-if="loading">⏳ 登录中...</span>
          <span v-else>🔑 微信登录</span>
        </button>

        <button @click="handleGetUserInfo" :disabled="loading || !authStore.token" class="btn info">
          <span v-if="loading">⏳ 获取中...</span>
          <span v-else>👤 获取用户信息</span>
        </button>

        <button @click="handleClearToken" class="btn danger">🗑️ 清除 Token</button>
      </div>
    </div>
      ...
      <div class="message-section">
        ...
        <div class="received-messages">
          ...
          <div class="message-list">
            ...
            <div v-if="receivedMessages.length === 0" class="no-messages">暂无消息</div>
          </div>
        </div>
      </div>
    </div>
...
<script setup lang="ts">
  ...
  const { isReady, isInMiniProgram, error, getMallToken, login, getUserInfo, on, off } =
    useWebViewBridge({
      debug: true,
      autoInit: true
    })
  ...
  const autoGetMallToken = async () => {
    ...
    try {
      ...
      // 显示失败提示
      showToast(
        `❌ Token 自动获取失败: ${err instanceof Error ? err.message : '未知错误'}`,
        'error'
      )
